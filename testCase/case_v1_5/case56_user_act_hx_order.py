...
guanjia_accounts_xls = common.get_xls("v1.5.xlsx", "act_hx_orders", tag=tag)
print 'excel里测试用例列表:\n', guanjia_accounts_xls


@paramunittest.parametrized(*guanjia_accounts_xls)
class GuanJiaHouTaiHx(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # 现金券状态改为未核销
        sql1 = "UPDATE coupon_lists SET used_status = 0 WHERE id =18;"
        configDB.MyDB().zhiyu_run_sql(sql1)

    def test_guanjia_houtai_hexiao(self):
        """
        test body
        :return:
        """
...
if __name__ == '__main__':
    GuanJiaHouTaiHx().test_guanjia_houtai_hexiao()
