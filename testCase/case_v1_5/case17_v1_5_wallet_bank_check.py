...
guanjia_accounts_xls = common.get_xls("v1.5.xlsx", "wallet_bank_check", tag=tag)
print 'excel里测试用例列表:\n', guanjia_accounts_xls


@paramunittest.parametrized(*guanjia_accounts_xls)
class WalletCheck(unittest.TestCase):
    ...
    def test_wallet_check(self):
        ...
        # 读取trac_no
        f1 = open(trac_no_path, 'r')
        trac_no_tmp = f1.readline()
        data['trac_no'] = trac_no_tmp
        # 判断是否需要token
        if self.token == 1:
