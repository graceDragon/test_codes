...
guanjia_accounts_xls = common.get_xls("v1.5.xlsx", "clean_dochange", tag=tag)
print 'excel里测试用例列表:\n', guanjia_accounts_xls


@paramunittest.parametrized(*guanjia_accounts_xls)
class CleanDoChange(unittest.TestCase):
    ...
    def test_clean_dochange(self):
        ...
        data['visit_date'] = common.time_to_str(data['visit_date'])
        # 判断是否需要token
        if self.token == 1:
...
if __name__ == '__main__':
    CleanDoChange().test_clean_dochange()
