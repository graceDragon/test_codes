# coding:utf-8
"""
v1.5-用户-优惠券相关-用户端推荐-提交推荐(执行数据库操作)
"""
import unittest
...
guanjia_accounts_xls = common.get_xls("v1.5.xlsx", "act_recommend_submit", tag=tag)
print 'excel里测试用例列表:\n', guanjia_accounts_xls


@paramunittest.parametrized(*guanjia_accounts_xls)
class UserActRecommendSubmit(unittest.TestCase):
    def setParameters(self, CaseName, CaseDescribe, Method, Token, ServiceID, Data, Result, ExpectState, ExpectMsg, SQL):
        """
        初始化excel表格里的数据
        set params
        :param case_name:
    ...
    def setUp(self):
        ...
        if self.sql_do == 1:
            sql = "DELETE FROM `coupon_recommend` WHERE mobile = '18211076666';"
    ...
    def test_user_act_recommend_submit(self):
        """
        test body
        :return:
        """
...
if __name__ == '__main__':
    UserActRecommendSubmit().test_user_act_recommend_submit()
