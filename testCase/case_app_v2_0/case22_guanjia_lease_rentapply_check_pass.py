...
guanjia_accounts_xls = common.get_xls("app_v2.0.xlsx", "lease_rentapply_docheck_pass", tag=tag)
print 'excel里测试用例列表:\n', guanjia_accounts_xls


@paramunittest.parametrized(*guanjia_accounts_xls)
class GuanJiaLeaseRentApplyDoCheckPass(unittest.TestCase):
    ...
    def test_lease_rentapply_docheck_pass(self):
        """
        test body
        :return:
        """
        ...
        data['data']['id'] = apply_id
        print '最新的申请编号：', data['data']['id']
        # 判断是否需要token
        if self.token == 1:
...
if __name__ == '__main__':
    GuanJiaLeaseRentApplyDoCheckPass().test_lease_rentapply_docheck_pass()
