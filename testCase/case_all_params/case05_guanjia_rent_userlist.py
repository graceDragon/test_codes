...
guanjia_accounts_xls = common.get_xls("all_params.xlsx", "guanjia_sign_userlist", tag=tag)
print 'excel里测试用例列表:\n', guanjia_accounts_xls


@paramunittest.parametrized(*guanjia_accounts_xls)
class GuanJiaRentUserList(unittest.TestCase):
    ...
    def test_rent_user_list(self):
        ...
        # 判断入参对应的返回数据
        str1 = '刘昱良'
        # if str1 in self.info['data']['lists'][0]['name'].encode('utf-8'):
        if str1 in self.info['data']['lists'][0].values():
...
if __name__ == '__main__':
    GuanJiaRentUserList().test_rent_user_list()
