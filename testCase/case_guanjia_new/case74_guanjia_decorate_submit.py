...
guanjia_accounts_xls = common.get_xls("guanjia_new.xlsx", "decorate_item_submit", tag=tag)
print 'excel里测试用例列表:\n', guanjia_accounts_xls


@paramunittest.parametrized(*guanjia_accounts_xls)
class GuanJiaDecorateSubmit(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if self.sql == 1:
            sql = "DELETE FROM `cp_decorate` WHERE NAME = '装修项目-自动化';"
    ...
    def test_rent_decorate_submit(self):
        """
        test body
        :return:
        """
...
if __name__ == '__main__':
    GuanJiaDecorateSubmit().test_rent_decorate_submit()
