...
guanjia_accounts_xls = common.get_xls("guanjia_accounts.xlsx", "add_client", tag=tag)
print 'excel里测试用例列表:\n', guanjia_accounts_xls


@paramunittest.parametrized(*guanjia_accounts_xls)
class GuanJiaAddClient(unittest.TestCase):
    ...
    def test_guanjia_add_client(self):
        """
        test body
        :return:
        """
    ...
    def tearDown(self):
        ...
        # 执行sql语句，添加的客户状态置为失效
        sql = localReadConfig.get_sql('sql_delete_customer')
        configDB.MyDB().zhiyu_delete(sql)


if __name__ == '__main__':
    GuanJiaAddClient().test_guanjia_add_client()
