...
guanjia_accounts_xls = common.get_xls("guanjia_accounts.xlsx", "electric_chargesubmit", tag=tag)
print 'excel里测试用例列表:\n', guanjia_accounts_xls


@paramunittest.parametrized(*guanjia_accounts_xls)
class GuanJiaElectricChargeSubmit(unittest.TestCase):
    ...
    def test_guanjia_electric_chargesubmit(self):
        """
        test body
        :return:
        """
        # 密码冻结需要时间，等待状态改变之后再次操作解冻，解冻也需要时间
        time.sleep(50)
...
if __name__ == '__main__':
    GuanJiaElectricChargeSubmit().test_guanjia_electric_chargesubmit()
